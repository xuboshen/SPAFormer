...
def get_shape_info(index, lev):
    # get children
    fn = "./PartNet/data_v0/" + str(index) + "/result_after_merging.json"
    root_to_load_file = []
    with open( fn, "r" ) as f:
    ...
    for part_objs in parts_objs:
        obj_fns = ["./PartNet/data_v0/" + str(index) + "/objs/" + obj +'.obj' for obj in part_objs]
        vs=[]
        fs=[]
...
if __name__=="__main__":
    ...
    for level in levels:
        ...
        for mode in modes:
            ...
            object_json =json.load(open(root_to_load_file + "/stats/train_val_test_split/" + cat_name +"." + mode + ".json"))
            object_list = [int(object_json[i]['anno_id']) for i in range(len(object_json))]

            #for each object:
            from tqdm import tqdm
            for i,fn in tqdm(enumerate(object_list)):
                # print("level ", level, " mode ", mode, "cat:", cat_name, " " ,fn," is start to convert!",i,"/",len(object_list))
                data_list.append(fn)
            np.save(root_to_save_file + cat_name + "." + str(mode) + ".npy", np.array(data_list, dtype=np.int64))
