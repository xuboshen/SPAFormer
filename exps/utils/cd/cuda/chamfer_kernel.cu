...
std::vector<at::Tensor> ChamferForward(
  ...
  TORCH_CHECK(xyz2.size(0) == batch_size);
...
std::vector<at::Tensor> ChamferBackward(
  ...
  TORCH_CHECK(grad_dist2.size(0) == batch_size);
  TORCH_CHECK(xyz1.size(0) == batch_size);
  TORCH_CHECK(xyz2.size(0) == batch_size);
  TORCH_CHECK(xyz1.size(1) == n1);
  TORCH_CHECK(xyz2.size(1) == n2);
  TORCH_CHECK(xyz1.size(2) == 3);
  TORCH_CHECK(xyz2.size(2) == 3);
  TORCH_CHECK(idx1.size(0) == batch_size);
  TORCH_CHECK(idx2.size(0) == batch_size);
  TORCH_CHECK(idx1.size(1) == n1);
  TORCH_CHECK(idx2.size(1) == n2);
  ...
  TORCH_CHECK(at::cuda::getApplyGrid(batch_size * n1, grid1, curDevice), 1, "Too many elements to calculate");
  TORCH_CHECK(at::cuda::getApplyGrid(batch_size * n2, grid2, curDevice), 1, "Too many elements to calculate");
