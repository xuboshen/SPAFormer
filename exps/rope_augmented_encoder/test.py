...
def test(conf):
    ...
    while val_batch_ind + 1 < val_num_batch:
        ...
        if val_batch_ind > conf.vis_number:
            return None
        ...
        with torch.no_grad():
            ...
            scores = 0
            part_cd_loss, shape_cd_loss, contact_point_loss, acc_num, valid_num, max_count, total_num, success_rate = forward(batch=val_batch, data_features=data_features, network=network, conf=conf, is_val=True, \
    ...
    total_sum_scores = sum_scores / val_num_batch
    ...
    utils.printout(conf.flog, f'total_shape_loss:{total_shape_loss.item()}')
...
def forward(batch, data_features, network, conf, \
    ...
    return total_cd_loss, shape_cd_loss, contact_point_loss, acc_num, valid_num, res_count, total_num, max(array_success_rate)
