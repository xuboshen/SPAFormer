...
class TransformerEncoder(nn.Module):
    ...
    def forward(self, src: Tensor, mask: Optional[Tensor] = None, src_key_padding_mask: Optional[Tensor] = None, each_layer_instance: Optional[Tensor] = None, return_intermedidate: Optional[Tensor] = None) -> Tensor:
        ...
        """
        output = src
        if each_layer_instance in [2]:
            if return_intermedidate == True:
                output_list = []
            instance_encoding = output[:, :, -20:]
            for ind, mod in enumerate(self.layers):
                output = mod(output, src_mask=None, src_key_padding_mask=src_key_padding_mask)
                if return_intermedidate == True:
                    output_list.append(output)

                output[:, :, -20:] = instance_encoding
...
class TransformerEncoderLayer(nn.Module):
    ...
    def __init__(self, d_model, nhead, dim_feedforward=2048, dropout=0.1, activation="relu", rope=None, multi_cat=0):
        super(TransformerEncoderLayer, self).__init__()
        self.self_attn = MultiheadAttention(d_model, nhead, dropout=dropout, rope=rope, multi_cat=multi_cat)
